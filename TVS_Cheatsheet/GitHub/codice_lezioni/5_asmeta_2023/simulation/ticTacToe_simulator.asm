...
definitions:
	...
	//la seguente proprietà riestra nel problema di EF su stati iniziali e la proprietà è Falsa
	// diversamente da come ci aspettiamo. Basta provare falsa la sua vegazione AG(not psi)
	CTLSPEC ef(endOfGame and not (winner(CROSS)) and not (winner(NOUGHT)))
	// negazione
	//CTLSPEC ag( not(endOfGame and not (winner(CROSS)) and not (winner(NOUGHT)) ) )
