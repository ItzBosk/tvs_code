...
public class RushHour {
	...
	// public invariant (\num_of int i; i >=0 && i < 6 && j >= 0 && j < 6; gameBoard[i][j] == 0) == 30;
	// le macchine sono 6
	// public invariant (\num_of int i, j; i >=0 && i < 6 && j >= 0 && j < 6; gameBoard[i][j] != 0) == 6;
	// la somma fa sempre 21
	//@ public invariant  (\sum int i; i >=0 && i < 6 ; (\sum int j; j >=0 && j < 6; gameBoard[i][j] )) == 21;
	// non esiste una cella con valore maggiore di 6
	//@ public invariant !(\exists int i; i >=0 && i < 6; (\exists int j; j >= 0 && j < 6; gameBoard[i][j] >6));
	// tutte le celle hanno valore maggiore o uguale a 0
	//@ public invariant (\forall int i; i >=0 && i < 6; (\forall int j; j >= 0 && j < 6; gameBoard[i][j] >=0));

	/*@ requires true;
	  @ ensures gameBoard[2][2]==1 && gameBoard.length == 6;
	  @*/	
	public RushHour() {
